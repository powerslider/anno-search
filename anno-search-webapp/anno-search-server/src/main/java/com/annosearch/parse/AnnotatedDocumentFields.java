package com.annosearch.parse;

/**
 * @author Tsvetan Dimitrov <tsvetan.dimitrov23@gmail.com>
 * @since 25-Jan-2017
 */
public class AnnotatedDocumentFields {

    public static final String STRING_FIELD = "string";
    public static final String CLASS_FIELD = "class";
    public static final String SUBCLASSES_FIELD = "subclasses";
    public static final String PREFERRED_LABEL_FIELD = "preferredLabel";
    public static final String TYPE_FIELD = "type";
    public static final String INDICES_FIELD = "indices";
    public static final String START_OFFSET_FIELD = "startOffset";
    public static final String END_OFFSET_FIELD = "endOffset";
    public static final String TEXT_ID_FIELD = "textId";
    public static final String TEXT_FIELD = "text";
    public static final String SENTIMENT_SCORE_FIELD = "sentimentScore";
    public static final String TITLE_FIELD = "title";
}
